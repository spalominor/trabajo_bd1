...
def crear_buques():
    ...
    esloras_random = rng.uniform(50, 400, size=ROWS)
    mangas_random = rng.uniform(10, 60, size=ROWS)
    calados_random = rng.uniform(5, 20, size=ROWS)
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        for i in range(ROWS):
            ...
            anio_construccion = int(anios_random[i])
            capacidad_teu = int(capacidades_random[i])
            eslora = int(esloras_random[i])
            manga = int(mangas_random[i])
            calado_maximo = int(calados_random[i])
            
            # foreign key to Naviera
            naviera_id = int(navieras_fk_random[i])
...
def crear_viajes():
    ...
    consumos_combustible_random = rng.uniform(1000, 20000, size=ROWS)
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        for i in range(ROWS):
            ...
            consumo_combustible = int(consumos_combustible_random[i])
            
            # FK to a Buque existing
            buque_id = int(buques_fk_random[i])
...
def crear_mercancias():
    ...
    pesos_random = rng.uniform(100.0, 50000.0, size=ROWS)
    volumenes_random = rng.uniform(1.0, 200.0, size=ROWS)
    paises_random = rng.choice(PAISES, size=ROWS)
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        for i in range(ROWS):
            ...
            peso_bruto = int(pesos_random[i])
            volumen = int(volumenes_random[i])
            pais_origen = paises_random[i]
            
            # FK to a Viaje existing
            viaje_id = int(viajes_fk_random[i])
